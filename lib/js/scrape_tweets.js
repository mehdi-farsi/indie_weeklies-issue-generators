...
async function scrapeTweetsFromUser(browser, username) {
  ...
  try {
    ...
    try {
      ...
      // Capture and save the HTML content of the page for debugging
      try {
        console.log(`[PARSE] Capturing HTML content of the page...`);
        const htmlContent = await page.content();

        // Save HTML to a file in the tmp directory
        const htmlFilePath = path.join(process.cwd(), 'tmp', `${username}_page.html`);
        fs.writeFileSync(htmlFilePath, htmlContent);
        console.log(`[PARSE] HTML content saved to ${htmlFilePath}`);

        // Also display a portion of the HTML in the console for immediate inspection
        const htmlPreview = htmlContent.substring(0, 1000) + '... (truncated)';
        console.log(`[PARSE] HTML preview:\n${htmlPreview}`);
      } catch (htmlError) {
        console.error(`[PARSE] Error capturing HTML content: ${htmlError.message}`);
      }
