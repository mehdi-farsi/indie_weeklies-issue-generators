...
async function extractFollowerCount(page) {
  try {
    // Wait for the followers element to load - try different possible selectors
    try {
      // First try the new format with verified_followers
      await page.waitForSelector('[href*="/verified_followers"]', { timeout: 3000 });
      const followerElement = await page.$('[href*="/verified_followers"]');

      if (followerElement) {
        // Extract the follower count from the element
        const followerText = await page.evaluate(el => el.textContent.trim(), followerElement);
        // Parse the number, handling formats like "188.5K"
        let followerCount = followerText.match(/[\d,.]+[KMB]?/i);
        if (followerCount) {
          followerCount = followerCount[0];
          // Convert K, M, B suffixes to actual numbers
          if (followerCount.includes('K')) {
            return parseFloat(followerCount.replace('K', '')) * 1000;
          } else if (followerCount.includes('M')) {
            return parseFloat(followerCount.replace('M', '')) * 1000000;
          } else if (followerCount.includes('B')) {
            return parseFloat(followerCount.replace('B', '')) * 1000000000;
          } else {
            return parseInt(followerCount.replace(/,/g, ''));
          }
        }
      }
    } catch (e) {
      // If the first selector fails, try the old format
      console.log("Trying alternative follower count selector...");
    }

    // Try the old format with /followers
    try {
      await page.waitForSelector('[href$="/followers"]', { timeout: 3000 });
      ...
      console.log("Alternative follower count selector also failed.");
    }

    // If all selectors fail, try a more generic approach
    try {
      // Look for any element containing "Followers" text
      const elements = await page.$$('a');
      for (const el of elements) {
        const text = await page.evaluate(el => el.textContent.trim(), el);
        if (text.includes('Followers')) {
          // Extract the number part
          let followerCount = text.match(/[\d,.]+[KMB]?/i);
          if (followerCount) {
            followerCount = followerCount[0];
            // Convert K, M, B suffixes to actual numbers
            if (followerCount.includes('K')) {
      ...
      console.log("Generic follower count extraction also failed.");
    }
